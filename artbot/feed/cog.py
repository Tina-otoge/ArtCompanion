...
from artbot import config, data, on_error, services
...
class FeedCog(commands.Cog):
    ...
    async def update_status(self):
        ...
        await self.bot.change_presence(
            activity=discord.Game(f"next feed {time.humanize()}")
        )
    ...
    async def loop(self):
        try:
            ...
            for index, rules in enumerate(feeds):
                ...
                await self.bot.change_presence(
                    activity=discord.Game(f"feed {index}")
                )
    ...
    async def handle(self, rules):
        ...
        for result in result.get("result", [])[: self.RESULTS_LIMIT]:
            ...
            try:
                ...
                if webhooks:
                    self.post_webhooks(
                        result, webhooks, rules.get("webhook_options", {})
                    )
            except Exception:
